...
public class AddProcessHandler(DatabaseRepo<Process> repo) : IRequestHandler<AddProcessCommand, IActionResult>
{
    public async Task<IActionResult> Handle(AddProcessCommand request, CancellationToken cancellationToken)
    {
        try
        {
            var process = await repo.CreateNew(request.Instance);
